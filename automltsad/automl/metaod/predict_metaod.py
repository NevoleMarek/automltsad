...
from automltsad.automl.metaod.models.gen_meta_features import (
    generate_meta_features,
)
from automltsad.automl.metaod.models.utility import fix_nan
...
def select_model(X, trained_model_location="trained_models", n_selection=1):
    ...
    # load PCA scalar
    meta_scalar = load(
        os.path.join(trained_model_location, "meta_scalar.joblib")
    )
    ...
    meta_X = meta_scalar.transform(np.asarray(meta_X).reshape(1, -1)).astype(
        float
    )
    ...
    # # load trained models
    model_lists = list(
        load(os.path.join(trained_model_location, "model_list.joblib"))
    )
