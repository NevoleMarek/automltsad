# Copyright (c) 2017 Alex Kofke
#
# github.com/akofke/autoperiod
# coding=utf-8
...
class Autoperiod(object):
    def __init__(
        ...
        times = np.array(list(range(values.size)))
        ...
        self._power_norm_factor = 1 / (
            2 * np.var(self.values - np.mean(self.values))
        )
        ...
        if period and is_valid:
            ...
            self._sinwave = (
                np.cos(2 * np.pi / self._period * (self.times - phase_shift))
                * amplitude
                + amplitude
            )
    ...
    def period_blocks(self):
        ...
        period_cutoff_indices = (
            np.where(period_region[:-1] != period_region[1:])[0] + 1
        )
        ...
        period_blocks = np.array_split(
            timeseries, period_cutoff_indices, axis=1
        )

        on_period_blocks = (
            period_blocks[::2] if period_region[0] else period_blocks[1::2]
        )
        off_period_blocks = (
            period_blocks[1::2] if period_region[0] else period_blocks[::2]
        )
    ...
    def period_block_areas(self):
        ...
        on_block_areas = np.array(
            [
                scipy.integrate.trapz(block[1], x=block[0])
                for block in on_period_blocks
            ]
        )
        off_block_areas = np.array(
            [
                scipy.integrate.trapz(block[1], x=block[0])
                for block in off_period_blocks
            ]
        )
    ...
    def period_area(self):
        ...
        on_period_area = integrate.trapz(
            self.values[period_region], self.times[period_region]
        )
    ...
    def _get_period_hints(self):
        ...
        for i, period in enumerate(self.periods):
            if (
                self.powers[i] > self._power_threshold
                and self.time_span / 2 > period > 2 * self.time_interval
            ):
                if i < (total_periods - 2) and i > 2:
                    period_hints.append((i, period))

        period_hints = sorted(
            period_hints, key=lambda p: self.powers[p[0]], reverse=True
        )
    ...
    def _mc_threshold(self):
        ...
        max_powers.sort()
        return (
            max_powers[int(len(max_powers) * 0.99)] * self._power_norm_factor
        )

    def _stat_threshold(self):
        return -1 * math.log(
            1 - math.pow(self._confidence_level, 1 / self.powers.size)
        )
    ...
    def validate_hint(self, period_idx):
        ...
        valid = min_slope1 > min_slope2 and not np.isclose(
            np.abs(angle1 - angle2), 0, atol=0.01
        )
        ...
        if self.plotter and (valid or self.plotter.verbose):
            self.plotter.plot_acf_validation(
                self.times,
                self.acf,
                self.times[search_min : t_split + 1],
                min_slope1,
                min_c1,
                min_stderr1,
                self.times[t_split : search_max + 1],
                min_slope2,
                min_c2,
                min_stderr2,
                t_split,
    ...
    def _get_acf_search_range(self, period_idx):
        min_period = 0.5 * (
            self.periods[period_idx + 1] + self.periods[period_idx + 2]
        )
        max_period = 0.5 * (
            self.periods[period_idx - 1] + self.periods[period_idx - 2]
        )
