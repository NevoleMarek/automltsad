...
from automltsad.metrics import excess_mass_auc_score, mass_volume_auc_score
...
def save_result(result):
    ...
    dir_path = MODEL_DIR + f'{detector}/unsupervised_ae/{dataset}/'
...
def main():
    ...
    with multiprocessing.Pool(MAX_WORKERS) as p:
        for result in tqdm.tqdm(
            p.imap_unordered(process_task, tasks, chunksize=1),
            total=len(tasks),
        ):
            save_result(result)
